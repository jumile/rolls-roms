...
import { nunjucksCompile } from 'gulp-nunjucks';  // нужно для автообновления сервера
...
/* эти пакеты нужны для поддержки @container
...
export function processStyles() {
  return src(`${PATH_TO_SOURCE}styles/styles.scss`, { sourcemaps: isDevelopment })
    .pipe(plumber())
...
export function createStack() {
  return src(`${PATH_TO_SOURCE}images/vector/**/*.svg`)
    .pipe(stacksvg())
    .pipe(dest(`${PATH_TO_DIST}images/vector`));
}
...
export function startServer() {
  ...
  watch(`${PATH_TO_SOURCE}images/vector/**/*.svg`, series(createStack, reloadServer));
